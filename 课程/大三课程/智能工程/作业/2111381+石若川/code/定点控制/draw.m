...
figure;
% 绘制散点图和光滑曲线
scatter(out.x_I.signals.values, out.y_I.signals.values, "filled"); % 绘制散点
hold on; % 保持图形并在同一图形中绘制下一个曲线
plot(xx, yy, '-', 'LineWidth', 1.5); % 绘制光滑曲线
grid on
...
hold on
quiver(x, y, cos(theta), sin(theta), 'Color','b', 'LineWidth',1.2,'AutoScaleFactor',4);  % 绘制箭头表示机器人方向
hold on
% 绘制垂直于XY轴的虚线
plot([x, x], [0, y], 'k--');
hold on
